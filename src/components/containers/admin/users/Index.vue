        ...
        <div class="left">
            <div class="display-flex space-between display-mobile margin margin-bottom-15px">
                <div class="width width-75 width-mobile display-flex space-between">
                    <h1 class="fonts big black bold">Users</h1>
...
export default {
    ...
    computed: {
        ...mapState({
            filter: (state) => state.storeUsers.filter,
            form: (state) => state.storeUsers.form,
            data: (state) => state.storeUsers.data,
            totalRecord: (state) => state.storeUsers.totalRecord,
            limit: (state) => state.storeUsers.limit,
            loading: (state) => state.storeUsers.loading,
            loadingForm: (state) => state.storeUsers.loadingForm,
            typeForm: (state) => state.storeUsers.typeForm
        }),
        typeForm: {
            get () {
                return this.$store.state.storeUsers.typeForm
            },
            set (value) {
                this.$store.state.storeUsers.typeForm = value
            }
        },
    },
    methods: {
        ...mapActions({
            ...
            setFormData: 'storeUsers/setFormData',
            createData: 'storeUsers/createData',
            updateData: 'storeUsers/updateData',
            deleteData: 'storeUsers/deleteData',
            uploadCover: 'storeUsers/uploadCover',
        }),
        onSearch (data) {
        ...
        onOpenVisibleConfirmed () {
            ...
            switch (this.typeForm) {
                case 'create':
                    this.titleConfirmed = 'Save this user ?'
                    break
                case 'edit':
                    this.titleConfirmed = 'Edit this user ?'
                    break
            }
        },
        ...
        onClickYesDelete () {
            ...
            this.deleteData({
                ...
                if (status === 'ok') {
                    ...
                    this.titleAlert = 'Failed to delete this user'
                }
            })
        },
